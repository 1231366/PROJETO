...
if(isset($_POST['btn_enviar'])) {
    ...
    $queryAtualizar = "UPDATE pacientes SET nome = '$nome', genero = '$genero', contacto = '$contacto', morada = '$morada', id_medico_responsavel = '$medicoResponsavel', id_enfermeiro_responsavel = '$enfermeiroResponsavel' WHERE id_paciente = $id_paciente";

    // Executar a consulta para atualizar o paciente
    if(mysqli_query($con, $queryAtualizar)) {
        // Redirecionar para a página inicial
        echo "<script>window.location.href = 'DoentesSec.php?sucess=2';</script>";
    } else {
...
<form action="EditPaciente.php?id=<?php echo $id_paciente ?>" method="POST">
      ...
      <select class="formbold-form-input" name="medico_responsavel" id="medico_responsavel" required>
        ...
        if ($result_medicos->num_rows > 0) {
          while ($row = $result_medicos->fetch_assoc()) {
            echo '<option value="' . $row["id_trabalhador"] . '"';
            if ($row["id_trabalhador"] === $medicoResponsavelDoPaciente) {
              echo ' selected';
            }
      ...
      <select class="formbold-form-input" name="enfermeiro_responsavel" id="enfermeiro_responsavel" required>
        ...
        if ($result_enfermeiros->num_rows > 0) {
          while ($row = $result_enfermeiros->fetch_assoc()) {
            echo '<option value="' . $row["id_trabalhador"] . '"';
            if ($row["id_trabalhador"] === $enfermeiroResponsavelDoPaciente) {
              echo ' selected';
            }
            echo '>' . $row["nome"] . '</option>';
          }
