            ...
            <li class="nav-item">
                ...
                <div id="collapse" class="collapse" aria-labelledby="heading" data-parent="#accordionSidebar">
                    <div class="bg-white py-2 collapse-inner rounded">
                        ...
                        <a class="collapse-item" href="indicadoresgravidas.php">Saúde Grávidas</a>
                    </div>
                </div>
            </li>
...
if (isset($_POST['btn_enviar'])) {
    ...
    $novo_material = $_POST['material'];
    $nova_quantidade = $_POST['quant'];

    // Atualizar o nome do material na tabela materiais
    $query_material = "UPDATE materiais SET material = '$novo_material' WHERE id_material = $id_material";
    $result_material = mysqli_query($con, $query_material);

    // Atualizar a quantidade na tabela inventario
    $query_inventario = "UPDATE inventario SET quantidade = $nova_quantidade WHERE id_material = $id_material";
    $result_inventario = mysqli_query($con, $query_inventario);

    if ($result_material && $result_inventario) {
        // Ambas as consultas foram executadas com sucesso
        // Redirecionar para a página desejada ou exibir uma mensagem de sucesso
        ?>

<script>
    // Redirecionar imediatamente
    window.location.href = 'GestaoMateriais.php?success=1';
</script>
<?php
        exit();
    } else {
        // Ocorreu um erro ao executar as consultas
        echo "Erro ao atualizar os dados: " . mysqli_error($con);
    }
} else {
    // Recuperar os dados existentes do material e quantidade
    $id_material = $_GET['id'];
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        // Recuperar a quantidade da tabela inventario
        $query_inventario = "SELECT quantidade FROM inventario WHERE id_material = $id_material";
        $result_inventario = mysqli_query($con, $query_inventario);
        $row_inventario = mysqli_fetch_assoc($result_inventario);
        $quantidade = $row_inventario['quantidade'];
    } else {
        // Registro do material não encontrado
        echo "Registro do material não encontrado.";
        exit();
    }
    ?>
<?php
include "ligacao.php";
// Recuperar os dados existentes do material, quantidade e ID do inventário
$id_material = $_GET['id'];

$query = "SELECT inventario.id_inventario, inventario.quantidade, materiais.material FROM inventario INNER JOIN materiais ON inventario.id_material = materiais.id_material WHERE inventario.id_material = $id_material";
$result = mysqli_query($con, $query);

if ($result) {
    $row = mysqli_fetch_assoc($result);
    // Agora você pode usar os valores obtidos
...
<form action="EditMaterial.php" method="POST" enctype="multipart/form-data">
  
      

  <div class="formbold-input-flex">
    <div>
      <label for="post" class="formbold-form-label"> Material </label>
      <input
        type="text"
        name="material"
        id="material"
    ...
    <div>
      <label for="city" class="formbold-form-label"> Quantidade </label>
      <input
        type="number"
        name="quant"
        id="quant"
...
<input type="submit" name="btn_enviar" class="formbold-btn" value="Editar Material">
</form>
