...
<body id="page-top">
    <?php
    session_start();
    if (!isset($_SESSION['nome']) || $_SESSION['cargo'] !== 'enfermeiro') {
...
.logo-image {
  ...
  margin: 0 auto; /* Para centralizar horizontalmente */
            ...
            <!-- Nav Item - Pages Collapse Menu -->
            <li class="nav-item">
                <a class="nav-link collapsed" href="GestaoStock.php">
                    <i class="fas fa-fw fa-folder"></i>
    ...
    <script>
        $(document).ready(function() {
            // Função para rolar a página até a tabela de equipamentos
            function scrollTable() {
                $('html, body').animate({
                    scrollTop: $('#equipamentosTable').offset().top
                }, 1000);
            }

            // Evento de clique no link do menu
            $('.nav-link').click(function() {
                // Verifica se o link corresponde à página de gestão de equipamentos
                if ($(this).attr('href') === 'GestaoStock.php') {
                    // Chama a função para rolar a página até a tabela de equipamentos
                    scrollTable();
                }
            });
        });
    </script>
                <a class="nav-link collapsed" href="GestaoStock.php#equipamentosTable">
                    <i class="fas fa-fw fa-folder"></i>
...
if (isset($_POST['submit1'])) {
    ...
    $atualizarDados = "UPDATE trabalhadores SET nome = '$nomecompleto', contacto = '$contacto', login = '$login' WHERE id_trabalhador = " . $_SESSION['id_trabalhador'];
...
<form action="DefinicoesUser.php" method="POST" enctype="multipart/form-data">
    ...
    <button class="btn btn-primary" type="file" id="upload" >Mudar Imagem</button>
</div>
</div>
</td>
</tr>
...
<button type="submit" id="submit" name="submit1" class="btn btn-primary">Atualizar</button>
</div>
</form>
...
?>
<form action="DefinicoesUser.php" method="POST">
		<div class="row gutters">
			<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
